window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function rerunEvaluation(workspaceId, sessionId) {
      const prior = recordFor(workspaceId);
      replaceRecord(workspaceId, { busy: true, error: "", errorCode: "" });
      try {
        const body = await apiJson("/api/wanxiang/evaluation/rerun", {
    ...
    function statusOf(record) {
      ...
      if (["can_make", "can_try", "can_shadow", "can_use"].includes(record.project.maturity?.stage)) {
        return record.project.maturity.label;
      }
    ...
    function MaturityPanel({ maturity }) {
      if (!maturity?.evidence) return null;
      const core = maturity.evidence.confirmedCoreFields;
      const representative = maturity.evidence.representativeCases;
      const accepted = maturity.evidence.acceptedRealRuns;
      const approval = maturity.evidence.approvalPolicy;
      const facts = [
        { key: "brief", label: "核心工作条件", value: `${core.confirmed}/${core.required} 项已确认`, satisfied: core.satisfied },
        { key: "cases", label: "代表案例", value: `${representative.passed}/${representative.total} 个通过，其中 ${representative.boundaryPassed} 个边界案例`, satisfied: representative.satisfied },
        { key: "real", label: "真实运行", value: `${accepted.total} 个已接受，连续 ${accepted.consecutive} 个`, satisfied: accepted.total >= accepted.requiredForTry },
        { key: "approval", label: "高风险动作", value: `${approval.source}：${approval.summary}`, satisfied: approval.satisfied },
      ];
      return h("section", { className: "wx-maturity", "aria-labelledby": "wx-maturity-title", "aria-live": "polite" },
        h("header", null,
          h("div", null, h("span", null, "当前阶段"), h("h3", { id: "wx-maturity-title" }, maturity.label))),
        h("h4", null, "成立依据"),
        h("ul", { className: "wx-maturity-evidence" }, facts.map((fact) => h("li", { key: fact.key, "data-satisfied": fact.satisfied || undefined },
          h("strong", null, fact.label), h("span", null, fact.value)))),
        maturity.next ? h("div", { className: "wx-maturity-next" },
          h("span", null, "下一阶段"),
          h("strong", null, maturity.next.label),
          h("ul", null, (maturity.next?.missing || []).map((item) => h("li", { key: item }, item)))) : null);
    }
    ...
    function RunEvidencePanel({ project, onRerun, rerunning, canRerun }) {
      ...
      return h("section", { className: "wx-evidence", "aria-labelledby": "wx-evidence-title", "aria-live": "polite" },
        ...
        h("button", {
          type: "button", className: "wx-rerun", disabled: rerunning || !canRerun,
          "aria-describedby": "wx-rerun-note", onClick: onRerun,
        }, rerunning ? "正在重跑全部案例…" : "手动重跑当前修订"),
        h("p", { id: "wx-rerun-note", className: "wx-evidence-note" },
          canRerun ? "本次会按当前 Workflow 版本运行全部代表案例。" : "请回到已开始制作的主会话后重跑。"),
        h("h4", null, "逐案例结果"),
          ...
          h("ol", null, [...runs].reverse().map((run) => {
            ...
            return h("li", { key: run.runId },
              ...
              run.evidence?.input ? h("details", { className: "wx-run-input" },
                h("summary", null, "输入快照"),
                h("pre", null, JSON.stringify(run.evidence.input, null, 2))) : null,
              run.evidence?.error?.message ? h("p", { role: "alert" }, run.evidence.error.message) : null,
              run.retryOf ? h("small", null, "前次运行 ", run.retryOf) : null);
          }))) : null);
    }
    function BriefPanel({ workspace, sessionId }) {
      ...
      const watchEvaluation = project.work.sessionId === sessionId
        && project.work.activeRevision === project.briefRevision;
      React.useEffect(() => {
        if (!watchEvaluation) return undefined;
      ...
      }, [workspaceId, watchEvaluation, record.busy]);
      ...
      return h("div", { className: "wx-brief-panel" },
        ...
        h(MaturityPanel, { maturity: project.maturity }),
        h(RealWorkPanel, {
    ...
    function ProductStyles() {
      return h("style", { "data-wanxiang-styles": "v3" }, `
        ...
        .wx-maturity{margin:0 0 18px;padding:14px;border:1px solid color-mix(in srgb,var(--dsw-alias-state-business-primary) 28%,var(--dsw-alias-border-l2));border-radius:14px;background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 5%,var(--dsw-alias-bg-base))}.wx-maturity header{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}.wx-maturity header span,.wx-maturity-next>span{display:block;color:var(--dsw-alias-label-tertiary);font-size:9px}.wx-maturity h3{margin:3px 0 0;color:var(--dsw-alias-state-business-primary);font:600 17px/1.4 ui-serif,"Songti SC","STSong",serif}.wx-maturity h4{margin:12px 0 7px;font-size:11px}.wx-maturity-evidence{list-style:none;margin:0;padding:0;display:grid;gap:5px}.wx-maturity-evidence li{display:grid;grid-template-columns:76px minmax(0,1fr);gap:7px;font-size:9px;line-height:1.5}.wx-maturity-evidence strong{color:var(--dsw-alias-label-tertiary)}.wx-maturity-evidence li[data-satisfied=true] strong{color:var(--dsw-alias-state-success-primary)}.wx-maturity-evidence span{color:var(--dsw-alias-label-secondary)}.wx-maturity-next{margin-top:11px;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l1)}.wx-maturity-next>strong{display:block;margin-top:2px;font-size:11px}.wx-maturity-next ul{margin:5px 0 0;padding-left:17px;color:var(--dsw-alias-label-secondary);font-size:9px;line-height:1.55}
