...
- 代理垂直切片：制作会话可用固定日期下的合成客户 CSV 与沟通 JSON，自动批量运行正常客户、超过 14 天未跟进、无沟通记录、高意向但无下一步、缺少负责人五个代表案例，也可从证据面板手动重跑当前修订；每个代理运行的状态和证据保留在 DSH Session、会话 Projection 和项目运行证据存储中。
